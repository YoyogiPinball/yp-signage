...
- **画面タイマーとリモコン** — 曜日ごとの予定で外部モニターだけを点灯・消灯し、ブラウザから画面やスライドショーを操作できます
...
画面タイマーは、曜日ごとに点灯する時間帯を決め、時間外は外部モニターの電源だけを切ります。
MagicMirror² は止めないため、モニターが点いた瞬間に元の画面が表示されます。

既定の点灯予定は次のとおりです。
`25:00` は、その日の深夜を越えた翌日 01:00 を表します。
...
`deploy.sh` で配っただけでは、画面タイマーは動き始めません。
配布後に表示機で次のコマンドを 1 回だけ実行し、ユーザーサービスを有効にしてください。
...
リモコンは、既定では表示機のブラウザで `http://127.0.0.1:8081` を開くと使えます。
...
表示機と操作する端末を同じ Tailnet につないだうえで、次の三つを順に済ませます。

まず、`timer.env` の `SIGNAGE_TIMER_USER` に自分の Tailscale ログイン名を書きます。
...
`--json` を付けない `tailscale status` の一覧にも出ますが、こちらは列幅で末尾が切れます。
`you@github` が `you@` に見えるため、そのまま書き写すと値が一致せず、リモコンはすべての要求に 403 を返します。
書き換えたら `systemctl --user restart signage-timer` で反映します。

次に、Tailnet 側で Serve を有効にします。
Tailscale の既定では無効で、この状態でコマンドを実行すると
`Serve is not enabled on your tailnet.` と管理画面の URL を表示したまま待ち続けます。
エラーで終了しないため設定は保存されず、コマンドを打ったのに何も起きていない状態になります。
表示された URL をブラウザで開いて有効にしてください。
...
`sudo` が要るのは、Tailscale の操作ユーザー（operator）を設定していない場合です。
毎回付けたくなければ、先に `sudo tailscale set --operator=$USER` を一度実行しておきます。
...
- 画面をつける
- 次の自動切り替えまで画面を消す、または明日の朝 7 時まで画面を消す
- 手動操作を取り消して自動運転に戻す
- スライドショーを一時停止・再開し、前後の画像へ送る
- 曜日ごとの点灯予定を編集する

点灯予定の編集画面では、日をまたぐ `25:00` を、終了時刻の `01:00` と「翌日」のチェックに分けて入力します。
ブラウザの時刻入力欄が `25:00` を扱えないためです。

画面の点灯と消灯には、GNOME (Mutter) の D-Bus プロパティ `PowerSaveMode` を使います。
Wayland では画面出力をコンポジタが管理しているため、`/sys` を直接書いても効かず、`xrandr` からは XWayland の外にある実画面へ届きません。
そこで、画面出力を持つ Mutter に直接変更を依頼しています。

サービスは 30 秒ごとに `PowerSaveMode` の実際の値を読み、予定と違っていれば当て直します。
`systemctl --user stop signage-timer` などでサービスを止めるときは、画面を点けてから終了します。
