...
- **背景** — 手元の画像フォルダを再帰的にスキャンし、60 秒ごとにフェードで切り替えます
...
画面を**右クリック**すると、次へ / 前へ / 一時停止 / 終了 のメニューが出ます。
表示機にキーボードとマウスが繋がっているなら、これがいちばん手軽です。左右の矢印キーでも送れます。
一時停止中は画面の右下に「一時停止中」のバッジが表示されます。
...
`legacy/` と `host/monitors.xml` は配布しません。いずれもリポジトリ内の記録用で、利用には関係ありません。
